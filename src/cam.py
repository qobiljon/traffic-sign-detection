...
model.load_state_dict(
    torch.load("../outputs/model.pth", map_location=device)["model_state_dict"]
)
...
def visualize_and_save_map(
    ...
    if class_idx is not None:
        cv2.putText(
            result,
            f"Pred: {str(class_names[int(class_idx)])}",
            (5, 20),
    ...
    if gt_idx is not None:
        cv2.putText(
            result,
            f"GT: {str(class_names[int(gt_idx)])}",
            (5, 40),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.55,
            (0, 255, 0),
    ...
    img_concat = cv2.hconcat(
        [np.array(result, dtype=np.uint8), np.array(orig_image, dtype=np.uint8)]
    )
